...
/**
 * The quick-add body — split out of the shell so `chrono-node` (the local
 * parser, ~330 KB of client JS) isn't in the critical path of every page in
 * the app. The shell warms this chunk on idle, so by the time anyone presses
 * Q it's already cached and the first keystroke still renders chips instantly.
 */
...
export default function QuickAddPanel({
  ...
  categories: Category[];
  /** Fires after a successful create; the shell shows the toast and closes. */
  ...
  async function confirm() {
    ...
    // Invalidate any in-flight refinement: a late response must never
    // resurrect a ghost draft after confirm.
    reqRef.current++;
    onDone(result.inbox ? "Captured to Inbox — schedule it any time." : "Added.");
  }
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <button
        ...
        className="-my-1 inline-flex min-h-6 items-center self-start text-xs text-ink-muted underline-offset-2 hover:text-ink hover:underline"
      >
